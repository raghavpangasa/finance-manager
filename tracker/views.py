...
def get_total_report():
    ...
    total_invested = MoneyTracker.objects.exclude(month="August",year=2021).aggregate(Sum('invested'))["invested__sum"]
    total_spent = MoneyTracker.objects.aggregate(Sum('expenses'))["expenses__sum"]
    total_saved = MoneyTracker.objects.aggregate(Sum('saved'))["saved__sum"]
    total_inhand = MoneyTracker.objects.aggregate(Sum('actual_inhand'))["actual_inhand__sum"]
...
def get_specific_data(selected_month, selected_year):
    try:
        tracker_object = MoneyTracker.objects.get(month=selected_month, year=selected_year)
        data = {
...
def get_investments_by_type(investments):
    result = dict()
    result["total_invested_amount"] = investments.aggregate(Sum('amount'))["amount__sum"]
    result["distribution"] = []
    for investment_type in INVESTMENT_TYPE:
        filtered_investments = investments.filter(investment_type=investment_type[0])
        investment_data = {
...
def get_expenses_by_tag(expenses):
    ...
    for tag in tags:
        ...
        amount_spent = filtered_expenses.aggregate(Sum('amount'))["amount__sum"]
        res.append({
            "name": tag.name,
...
def get_expense_summary(request):
    ...
    for monthly_tracker in  all_data:
        ...
        data_key = monthly_tracker.month + " - " + str(monthly_tracker.year)
        var_key = monthly_tracker.month.lower() + "_" + str(monthly_tracker.year)
        total_spent_amount = filtered_expenses.aggregate(Sum('amount'))["amount__sum"]
